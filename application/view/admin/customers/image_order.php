...
<?php
$i=1;
$customer=$call_config->get_all("SELECT * FROM `tbl_image_order_master` where `cid`='".$id."' order by `con`");
foreach ($customer as $key) {
  $address=$call_config->get("SELECT * FROM `tbl_address_master` WHERE id='".$key['addressid']."' ");
  ?>
<tr><td><?= $i++ ?> </td>
  <td><a class="btn btn-info mt-3" target="_blank" href="<?php echo $call_config->base_url().$key['attachment'] ?>">View</a></td>
