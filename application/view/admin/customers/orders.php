...
<?php
$i=1;
$customer=$call_config->get_all("SELECT * FROM `tbl_parent_order_master` where `cid`='".$id."' order by `con`");
foreach ($customer as $key) {
  $address=$call_config->get("SELECT * FROM `tbl_address_master` WHERE id='".$key['addressid']."' ");
  ?>
<tr><td><?= $i++ ?> </td>
  <td><a target="_blank" href="<?= $call_config->base_url()?>application/view/admin/customers/view_order.php?id=<?= $key['id']?>" class="btn btn-info btn-sm">View</a></td>
