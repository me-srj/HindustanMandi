...
<div class="table-responsive">
    <table class="table-striped table">
        ...
        <tbody>
<?php
$parent_order=$call_config->get_all("SELECT * FROM `tbl_parent_order_master` WHERE d_date is null AND exp_d_date is not null AND status='1' order by exp_d_date");
...
<td><?=  $address['address']."<br>".$address['city'].",".$address['state'].",".$address['mobile'].",".$address['pin_code'] ?></td>
<td><a target="_blank" href="<?= $call_config->base_url()?>application/view/admin/in_process_order/view_order.php?id=<?= $key['id']?>" class="btn btn-info btn-sm">View</a></td>
<td>
<?php echo date('d-M-Y',strtotime($key['exp_d_date'])) ?>
<td><button type='submit' name="submit" class="btn btn-success btn-sm">Delivered</a></td>
</tr>
</form>
<?php
}
