...
$name=mysqli_escape_string($call_config->con,$_POST["name"]);
	$sql="SELECT * FROM `tbl_product_master` WHERE  `name` LIKE '%".$name."%' OR `mrp` LIKE '%".$name."%' OR `selling_price` LIKE '%".$name."%'";
//$mysql=mysqli_query($call_config->con,$sql);
...
?>
	...
	<a class='btn btn-warning btn-sm' href='<?= $call_config->base_url()."application/modal/admin/delete_product.php?id=".$key['id'] ?>'>Delete</a>
